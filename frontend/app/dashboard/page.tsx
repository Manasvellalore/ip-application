...
const FALLBACK_BACKEND_URL = "https://test-app-backend-uyyi.onrender.com";
...
export default function Dashboard() {
  ...
  const fetchAdvancedIP = async (ip: string) => {
    ...
    if (!apiKey) {
      setIpAdvError("Missing API key: set NEXT_PUBLIC_IP_ADV_GENERATED_API_KEY or NEXT_PUBLIC_GENERATED_API_KEY");
      return;
    }
            ...
            {!ipAdvHasResult && !ipAdvError ? (
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="min-w-0">
                  <h4 className="text-amber-400/90 text-[11px] font-semibold uppercase tracking-wide mb-2">
                    Blacklist
                  </h4>
                  <pre
                    className="max-h-[min(55vh,420px)] overflow-auto rounded-lg border border-amber-500/25 bg-black/60 p-4 text-left font-mono text-[13px] leading-relaxed text-gray-200 shadow-inner [tab-size:2]"
                  >
                    {ipAdvBlacklistJson}
                  </pre>
                </div>
                <div className="min-w-0">
                  <h4 className="text-sky-400/90 text-[11px] font-semibold uppercase tracking-wide mb-2">
                    Information
                  </h4>
                  <pre
                    className="max-h-[min(55vh,420px)] overflow-auto rounded-lg border border-sky-500/25 bg-black/60 p-4 text-left font-mono text-[13px] leading-relaxed text-gray-200 shadow-inner [tab-size:2]"
                  >
