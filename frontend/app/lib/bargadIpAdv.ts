...
export const resolveIpAdvApiKey = () =>
  process.env.NEXT_PUBLIC_IP_ADV_GENERATED_API_KEY?.trim() ||
  process.env.NEXT_PUBLIC_GENERATED_API_KEY?.trim() ||
  "";
...
export async function fetchIpAdvForIp(ip: string, apiKey: string) {
  ...
  if (!apiKey) {
    return {
      ...
      errorMessage: "Missing API key (NEXT_PUBLIC_GENERATED_API_KEY or NEXT_PUBLIC_IP_ADV_GENERATED_API_KEY)",
    };
  }
  if (!trimmed) {
    return {
