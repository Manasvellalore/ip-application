...
function IpCheckContent() {
    ...
    const id = window.setTimeout(() => {
      ...
      void (async () => {
        try {
          ...
          if (cancelled) return;
          console.error("IP check load error:", err);
          setPartitions(null);
          setIpMapCoords(null);
          setError(
            "Could not reach the API. From the backend folder run: npm run start:dev (Nest on port 3001). Or set NEXT_PUBLIC_BACKEND_URL to your API base URL.",
          );
        } finally {
          if (!cancelled) {
            setLoading(false);
  ...
  const coordsAndAddressClipboardText = useMemo(() => {
    if (!ipMapCoords) return "";
    const coordLine = `Coordinates: ${ipMapCoords.lat.toFixed(6)}, ${ipMapCoords.lng.toFixed(6)}`;
    const addressDisplay =
      ipMapAddressLoading && !ipMapAddressLine
        ? "Resolving address…"
        : ipMapAddressLine ||
        "Could not resolve address (check NEXT_PUBLIC_GENERATED_API_KEY).";
    const addrLine = `Address: ${addressDisplay}`;
    return `${coordLine}\n${addrLine}`;
  }, [ipMapCoords, ipMapAddressLine, ipMapAddressLoading]);
            ...
            {ipParam && mapKey ? (
              <div className="mt-6 space-y-3">
                {loading ? (
                  ...
                  <>
                    <div className="relative rounded-lg border border-white/10 bg-black/30 py-2 pl-3 pr-12 text-xs leading-relaxed text-gray-200">
                      <p>
                      ...
                      <ClipboardCopyButton
                        text={coordsAndAddressClipboardText}
                        className="absolute right-2 top-2 rounded-md border border-[#24aa4d]/40 p-1.5 text-[#5edd7c] transition-colors hover:bg-[#24aa4d]/15 disabled:pointer-events-none disabled:opacity-30"
                      />
                    </div>
                    <MapplsIndiaMap
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="min-w-0">
              ...
              <div className="relative">
                <ClipboardCopyButton
                  text={blacklistJson}
                  className="absolute right-4 top-3 z-10 rounded-md border border-amber-500/40 bg-black/80 p-1.5 text-amber-400/90 transition-colors hover:bg-amber-500/10"
                />
            ...
            <div className="min-w-0">
              ...
              <div className="relative">
                <ClipboardCopyButton
                  text={informationJson}
                  className="absolute right-4 top-3 z-10 rounded-md border border-sky-500/40 bg-black/80 p-1.5 text-sky-400/90 transition-colors hover:bg-sky-500/10"
                />
